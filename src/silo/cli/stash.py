...
@click.group(cls=ColorGroup, help="Manage stashed changes", invoke_without_command=True)
@click.pass_context
def stash(ctx):
...
@stash.command("save", help="Stash modified files under a name")
@click.argument("name")
@click.option("--noignore", is_flag=True, help="Ignore .siloignore and stash all files")
def stash_save(name, noignore):
    silo_dir = require_silo()
    if not silo_dir:
        return
